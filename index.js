...
const boxOut = require('boxed-out/index');
const utilStringWrap = require('util-string-wrap');
const eachProp = objEachBreak.eachProp;
const toIntegerOrDefault = utilTypeFuncs.toIntegerOrDefault;
const toArrayIndexOrDefault = utilTypeFuncs.toArrayIndexOrDefault;
...
const isObject = utilTypeFuncs.isObject;
...
// Here this is the BoxedIn or the BoxedOut instance which are pure objects since they don't
// need inheritance
const RESERVED_PROPS = {
    _$: {  // BOXED_INSIDE, request boxedIn from parent
        getBoxedIn: function () {return this.boxedInProxy;},   // for performance optimization
                                                               // this is handled by proxy
                                                               // handler
        getBoxedOut: function () {return this.boxedInProxy;},  // for performance optimization
                                                               // this is handled by proxy
                                                               // handler
        ...
        ownPropertyDescriptor: function () {
            return {
                value: this.boxedInProxy,
                writable: true, /*enumerable: false, configurable: false, get:UNDEFINED, set:UNDEFINED,*/
            };
        },
    },
    $_: { // BOXED_OUTSIDE: request boxedOut from parent
        getBoxedIn: function () {return this.getBoxedOutValue();},
        getBoxedOut: function () {return this.getBoxedOutValue();}, // withBoxedOut is an
                                                                    // object, cannot return
                                                                    // ref to self, otherwise
        ...
        ownPropertyDescriptor: function () {
            return {
                value: this.getBoxedOutValue(),
                writable: !!this.boxedOutProxy, /*enumerable: false, configurable: false, get:UNDEFINED, set:UNDEFINED,*/
            };
        },
    },
    $_forEachKey: {
    ...
    $_array: {
        getBoxedIn: function () {return this.valueArrayBoxedOut();},
        getBoxedOut: function () {return this.valueArrayBoxedOut();},
        set: function (value) {return false;},
        delete: function () {return false;},
        ownPropertyDescriptor: function () {return { value: this.valueArrayBoxedOut() /*writable: false, enumerable: false, configurable: false, get:UNDEFINED, set:UNDEFINED,*/ };},
    },
    $_ifObject: {
    ...
    $_object: {
        getBoxedIn: function () {return this.valueObjectBoxedOut();},
        getBoxedOut: function () {return this.valueObjectBoxedOut();},
        set: function (value) {return false;},
        delete: function () {return false;},
        ownPropertyDescriptor: function () {return { value: this.valueObjectBoxedOut() /*writable: false, enumerable: false, configurable: false, get:UNDEFINED, set:UNDEFINED,*/ };},
    },
    ifObject_$: {
    ...
    default_$: {
        ...
        ownPropertyDescriptor: function () {
            return {
                value: this.setValueOfBoxedInDefault,
                writable: true, /*enumerable: false, configurable: false, get:UNDEFINED, set:UNDEFINED,*/
            };
        },
    },
    $_default: {
        ...
        ownPropertyDescriptor: function () {
            return {
                value: this.setValueOfBoxedOutDefault,
                writable: true, /*enumerable: false, configurable: false, get:UNDEFINED, set:UNDEFINED,*/
            };
        },
    },
    path_$: {
        ...
        ownPropertyDescriptor: function () {
            return {
                value: this.boxedInOfPath,
                writable: false, /*enumerable: false, configurable: false, get:UNDEFINED, set:UNDEFINED,*/
            };
        },
    },
    $_path: {
...
function  defaultCanBox(value) {
    return !isObjectLike(value) || isObject(value) || isArray(value);
}

module.exports.defaultCanBox = defaultCanBox;

const BOX_ONLY_PROPERTIES = {
    setTransforms: null,
    getTransforms: null,
    canBox: defaultCanBox,
};
...
function BoxContext(options, parent) {
    ...
    if (parent) {
        ...
        this.canBox = parent.context.canBox;
    } else {
        this.globalBox = UNDEFINED; // will be set by box
        this.canBox = options.canBox || defaultCanBox;
    }
    ...
    if (!magicProps.hasOwnProperty(magicParamsKey)) {
...
BoxContext.prototype.boxedContext = function (options) {
    return !hasOwnProperties.call(options) ? this : new BoxContext(options, this);
};
...
function boxedContext(options) {
    if (isObjectLike(options) && options.constructor === BoxContext) return options;
...
function setBoxedOutProxy(box) {
    if (isObjectLike(box.withBoxedOut)) {
...
function boxedProxy(box) {
    ...
    // need this to have the proxy stop yapping about it, the target is trying to be an object
    // or array
    Object.defineProperty(box.withBoxedIn, "length", {
        value: 0,
        writable: true,
        configurable: false,
        enumerable: false,
...
function getWithBoxedOutType(value) {
    let arr;
    return isObjectLike(value) && (value.constructor === Object || (arr = value.constructor === Array)) ? arr ? Array : Object : UNDEFINED;  // pure objects for proxy use
}

function getWithBoxedOut(value) {
    // pure objects for proxy use
    return isObjectLike(value) ? isArray(value) ? boxOut([]) : boxOut({}) : UNDEFINED;
}
...
function Box(value, parent, prop, options, boxOptions) {
    ...
    // really need to have this be set to the same type of object as the value, if the value
    // does not exist then can be anything, but as soon as value is changed this should also
    // change to reflect a new type so that type comparison for object or array will at least
    // match.
    this.withBoxedOut = getWithBoxedOut(this.value);

    // only root level box applies the get transform and only on freshly loaded value,
    // transforms it and pretends it is not modified.
    if (isObjectLike(this.getTransforms)) {
 ...
 *                  function - callback for taking proxy as argument for ._$(_$ => {}), return
 *     value returned by the callback function anything else - set the value and return boxed
 *     proxy for chaining
 *
...
Box.prototype.getBoxedOutValue = function getBoxedOutValue() {
...
Box.prototype.has = function (target, prop) {
    ...
    return isObjectLike(this.value) ? this.value.hasOwnProperty(prop) : false;
};
...
Box.prototype.getOwnPropertyDescriptor = function (target, prop) {
    ...
    if (isObjectLike(this.value) || isFunction(this.value)) {
...
/**
 * Convert an array of transforms or an individual transform to an object with transform
 * properties mapping
 * @param   arguments
...
// except if value canBox is false
Box.prototype.getBoxedProp = function (prop) {
    if (prop in this.boxedProps) {
        return this.boxedProps[prop];
    }

    let value = isObjectLike(this.value) ? this.value[prop] : UNDEFINED;

    // pass transforms to child if our transforms for the property are an object with matching
    // property name
    let setTransforms;
    // let getTransforms;
    let transforms;
    ...
    if (this.context.canBox(value, prop, this.value)) {
...
Box.prototype.getBoxedIn = function (prop) {
    ...
    return boxed instanceof Box ? boxed.boxedInProxy : boxed;
};
...
Box.prototype.getBoxedOut = function (prop) {
    ...
    if (this.withBoxedOut) {
        ...
        if (getWithBoxedOutType(element)) {
            // pass through all non-pure Objects so that anything can be stored in the property
            // and accessed via boxedOut
            let boxed = this.getBoxedProp(prop);
            return boxed instanceof Box ? boxed.boxedInProxy : boxed;
        }
        return element;
...
Box.prototype.copyOfValue = function copyOfValue() {
    return cloneArrayObject.call(this.value);
};
...
Box.prototype.boxedOfPath = function boxedOfPath(isBoxed, path) {
    if (isString(path)) {
            ...
            for (let i = 0; i < iMax; i++) {
                ...
                if (part !== this.boxedArrayEnd) {
                    if (this.reservedProps.hasOwnProperty(part)) {
                        throw new TypeError("InvalidArgument, path parts cannot be reserved property names (other than " + this.boxedArrayEnd + "), got " + part + " in " + path);
                    }

                    lastPart = part;

                    if (boxed instanceof Box) {
                        lastCollection = undefined;
                        boxed = boxed.getBoxedProp(part);
                    } else {
                        lastCollection = boxed;
                        boxed = boxed[part];
                    }
                }
            }
            ...
            if (args > 2) {
                ...
                if (isFunction(value)) {
                    ...
                    if (args > 3) {
                        ...
                        value = func.apply(boxed instanceof Box ? (isBoxed ? boxed.getBoxedOutValue() : boxed.value) : boxed, argList);
                    } else {
                        // just pass it as arg and set to its return
                        value = func(boxed instanceof Box ? (isBoxed ? boxed.boxedInProxy : boxed.value) : boxed);
                    }
                }
                ...
                if (boxed instanceof Box) {
                    ...
                    if (lastCollection === undefined || lastPart === undefined) {
                        throw "IllegalState, lastCollection or lastPart is undefined and value is unBoxable, should not happen"
                    }
                    lastCollection[lastPart] = value;
                }
            }
            return boxed;
        }
        ...
        throw new TypeError("InvalidArgument, only string path allowed, got " + path);
    }
};
...
function applyTransform(transforms, defaultTransforms, boxedArrayEnd, key, value, oldValue, getValue, setValue, applyFunctions, applyObjects) {
    if (transforms || defaultTransforms) {
        ...
        for (let j = 0; j < jMax; j++) {
            ...
            if (transform) {
                if (applyObjects && isObjectLike(transform)) {// recurse
...
function applyTransforms(value, transforms, boxedArrayEnd, applyFunctions, applyObjects, markModified) {
    ...
    function setValue(prop, propValue) {
        if (value[prop] !== propValue) {
            if (!isCopy) {
                value = cloneArrayObject.call(value);
                isCopy = true;
            }
    ...
    if (defaultTransform) {
        // may have keys there were not processed above because they only have default
        // transforms
...
Box.prototype.getValueProp = function getValueProp(prop) {
    ...
    return isObjectLike(this.value) && this.value.hasOwnProperty(prop) ? this.value[prop] : undefined;
};
...
Box.prototype.setValueProp = function setValueProp(prop, value) {
    ...
    } else if (!isObjectLike(this.value) || this.value[prop] !== value) {
...
Box.prototype.set = function (prop, value, boxed) {
    if (boxed) {
        if (boxed !== this.boxedProps[prop]) {
            ...
            throw new TypeError("Boxed property mismatch");
        }
    }

    if (prop === this.globalBox) prop = this.boxedArrayEnd;
    const isArrayEnd = prop === this.boxedArrayEnd;
    let isNumeric = isArrayEnd;
    ...
    if (!isArrayEnd) {
        ...
        let n = toArrayIndexOrDefault(prop);
        isNumeric = n !== UNDEFINED || isArrayEnd;
        if (isNumeric) {
            prop = n;
        }
    }
    ...
    const oldValue = isObjectLike(this.value) ? this.value[prop] : undefined;
    if (this.setTransforms) {
        const transform = this.setTransforms[prop];
        const defaultTransform = this.setTransforms[this.boxedArrayEnd];
        if (transform || defaultTransform) {
            if (isArrayEnd) {
                prop = arrayLength.call(this.value);
            }
            ...
            // get the potentially transformed value, if it is changed then it boxed prop is
            // invalid
    ...
    if (!this.isCopy || !isObjectLike(this.value)) {
        if (canOptimizeSameValue && isObjectLike(this.value) && oldValue === value) return true;

        // value being modified, have to make a copy or create a new value since this one will
        // have properties
        if (isNullOrUndefined(this.value) || !isObjectLike(this.value)) {
            ...
            this.isFullCopy = true; // all modifications will be reflected in the value, no
                                    // need to track mods
        } else {
    ...
    if (isArrayEnd) {
        if (prop === this.boxedArrayEnd) prop = arrayLength.call(this.value);
    ...
    if (this.extraValues) {
        ...
        eachProp.call(this.extraValues, (value, key) => {
...
Box.prototype.delete = function (prop) {
    ...
    if (isObjectLike(this.value) && this.value.hasOwnProperty(prop)) {
        ...
        if (delete newValue[prop]) {
            ...
            if (this.deleteEmptyCollections && !hasOwnProperties.call(this.value)) {
                // no props left then ask parent to delete our property
                if (this.prop && this.parent) {
                    // update self value in parent, without invalidating this boxed version
                    if (!this.parent.delete(this.prop)) {
                        // can't delete, update self value in parent, without invalidating this
                        // boxed version
                        this.parent.set(this.prop, this.value, this);
                    }
                }
...
Box.prototype.setValueOf = function (value) {
    // set value to new value as if it was unmodified, detach all boxed props, clear isCopy if
    // value not already equal
    if (isObjectLike(this.setTransforms)) {
...
function defaultOptions(options) {
    return options === 'shallow' && 1 || (options === 'deep' || options === 'merge') && Number.MAX_SAFE_INTEGER || toIntegerOrDefault(options);
}
...
Box.prototype.setValueOfDefaultImpl = function (boxedIn, options, value) {
    ...
    if (value !== UNDEFINED) {
        ...
        else if (isObjectLike(this.value) && typeof this.value === typeof value && options) {
            let levels = defaultOptions(options);
            if (levels === UNDEFINED && isObjectLike(options)) levels = defaultOptions(options['levels']);
            if (levels > 0) {
                if (isArray(value)) {
                    ...
                    eachProp.call(value, (keyValue, key) => {
                        const valueProp = this.value[key];
                        if (valueProp === UNDEFINED) {
                            // just set it
                            this.set(key, keyValue);
                        } else if (levels > 1) {
                            if (isObjectLike(valueProp) && typeof keyValue === typeof valueProp) {
                                // pass it to boxed
...
// if value is object or array: return only first level children that were modified or
// UNDEFINED if no mods if otherwise same as valueOfModified return value if modified or
// UNDEFINED if not modified here we do not copy
Box.prototype.unboxedDelta = function () {
    if (!this.isCopy || this.isFullCopy || !isObjectLike(this.value)) {
        return this.valueOfModified();
    }

    // we will copy out only modified values
    // arrayDeltaObjects: when true array deltas will be copied as objects so no erroneous
    // undefs arrayDeltaPartials: when true array deltas will contain undefined for all
    // unmodified fields or deleted fields
    let isArrayDelta = !this.context.arrayDeltaObjects && !this.context.arrayDeltaPartials && isArray(this.value);
    let delta = !isArrayDelta ? {} : this.value.constructor(); // always something or we would
                                                               // not be here
    ...
    if (isArrayDelta) {
        ...
        while (i--) {
            ...
            if (n !== UNDEFINED && maxIndex === UNDEFINED || n > maxIndex) {
                ...
                // TEST: that undefined values are not added to delta && deepDelta unless
                // this.ignoreUndefinedProperties is false
...
Box.prototype.setUnboxedDelta = function (value) {
    if (!isObjectLike(this.value) || !isObjectLike(value)) {
        return this.setValueOf(value);
    }
    ...
    if (isPureArrayDelta) {
        ...
        while (i--) {
            ...
            if (n !== UNDEFINED && maxIndex === UNDEFINED || n > maxIndex) {
                ...
                let value = this.value[prop];
                // TEST: undefined values are not added to delta && deepDelta unless
                // this.ignoreUndefinedProperties is false
                if (value !== UNDEFINED || !this.ignoreUndefinedProperties) delta[prop] = value;
            }
        }
...
Box.prototype.setUnboxedDeepDelta = function (value) {
    ...
    while (i--) {
        ...
        if (!isObjectLike(propValue)) {
            ...
            const boxedProp = this.getBoxedProp(prop);
            if (boxedProp instanceof Box) {
                boxedProp.setUnboxedDeepDelta(propValue);
            } else {
                throw `IllegalArgument, deepDelta path has unBoxable destination at property ${prop}`;
            }
        }
    }
    return true;
 ...
 *
 *              first arg is object, then assumed to hold options:
 *                  props: default false, true to have arrays include non-numeric keys
 *                  undefs: default false, true to have keys and array indices with undefined
 *     values included
 *
 *              arg: strings & numbers are keys of interest, will be pre-filtered, saves on
 *     creating a proxy for undesired properties arg; object then its keys will be added to
 *     keys of interest arg: array, contains args other than callback function
 *
...
function forEachKeyImpl(boxedIn, args) {
    ...
    if ((isArray(args) || typeof args === typeof arguments) && isObjectLike(value)) {
            ...
            if (argCount) {
                ...
                if (argCount) {
                    ...
                    while (i--) {
                        let arg = args[i];
                        if (isObjectLike(arg)) {
                            // take its keys on first level
                            if (isArray(arg)) filteredKeys.concat(arg);
                            else filteredKeys.concat(Object.keys(arg));
                        } else if (wantIntegers && (n = toArrayIndexOrDefault(arg))) {
                            filteredKeys.push(n);
                        } else {
            ...
            if (filteredKeys) {
                ...
                while (i--) {
                    ...
                    if (value.hasOwnProperty(prop)) {
                        if (isArrayValue) prop = toArrayIndex(prop);
...
Box.prototype.ifValueValidBoxedOut = function (callback) {
    return this.ifImpl(false, isValid(this.value), arguments);
};

Box.prototype.ifValueValidBoxedIn = function (callback) {
    return this.ifImpl(true, isValid(this.value), arguments);
};

Box.prototype.ifValueArrayBoxedOut = function (callback) {
    return this.ifImpl(false, isArray(this.value), arguments);
};

Box.prototype.ifValueArrayBoxedIn = function (callback) {
    return this.ifImpl(true, isArray(this.value), arguments);
};

Box.prototype.ifValueObjectBoxedOut = function (callback) {
    return this.ifImpl(false, isObjectLike(this.value), arguments);
};

Box.prototype.ifValueObjectBoxedIn = function (callback) {
    return this.ifImpl(true, isObjectLike(this.value), arguments);
};

Box.prototype.valueArrayBoxedOut = function (callback) {
    if (isArray(this.value)) {
        // this is just the boxed out value
    } else {
        // if invalid then new array, otherwise wrap value in array
        if (isValid(this.value)) {
            // wrap it
            this.setValueOf([this.value]);
        } else {
            this.setValueOf([]);
        }
    }
    return this.getBoxedOutValue();
};

Box.prototype.valueObjectBoxedOut = function (callback) {
    if (isObjectLike(this.value) && !isArray(this.value)) {
        // this is just the boxed out value
    } else {
        // otherwise if array and not empty, convert to object  or new object
        if (isArray(this.value)) {
            // convert it
            this.setValueOf(Object.assign({}, this.value));
        } else {
            this.setValueOf({});
        }
    }
...
function isBox(target) {
    return isObjectLike(target) && target.constructor === Box;
}
...
function throwError(box) {
    if (box === UNDEFINED) {
        throw new TypeError("BoxedHandler: IllegalArgument expected box function target");
    } else {
        throw new TypeError("BoxedHandler: IllegalState boxedOut proxy is already detached, probably held on to a boxedIn proxy after the type of the property value was changed");
    }
}
...
function isPrefixedToBoolean(arg, prop) {
    return startsWith(prop, 'is') ? !!arg : arg;
}
...
module.exports.getBoxOnlyOptions = getBoxOnlyOptions;

function getBoxOnlyOptions(options) {
    return objEachBreak.copyFiltered.call(undefined, options, BOX_ONLY_PROPERTIES);
}

function createBox(options) {
    let context = new BoxContext(options);
    let boxOnlyOptions = getBoxOnlyOptions(options);
    ...
    let boxedIn = function boxedIn() {
        ...
        let box;
        if (isObjectLike(boxOptionOverride)) {
